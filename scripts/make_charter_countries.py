...
def from_source():
    try:
        with open(SOURCE,newline='',encoding='utf-8') as f:
            r=csv.DictReader(f); keep=[c for c in r.fieldnames if c.lower() in {'country','code','cg','hr','region','year'}]
            if not keep: return None
            rows=[keep]
            for row in r: rows.append([row.get(k,'') for k in keep])
            return rows
...
def demo_rows():
    random.seed(42)
    hdr=['country','code','region','cg','hr','year']
...
def ensure_csv():
    ...
    if os.path.exists(TARGET):
        print('Found existing data/charter_countries.csv — skip'); return
